...
class Clone:
    def __init__(self, cells: List[Cell]):
        ...
        self.read_counts = sum((Counter(c.read_counts) for c in cells), Counter())
        self.n = len(cells)
...
class CloneGraph:
    ...
    def _precluster_cells(cells):
        ...
        for cell in cells:
            clone_ids = tuple(sorted(cell.read_counts))
    ...
    def _make_graph(self):
        ...
        """
        clones = [clone for clone in self._clones if clone.read_counts]
        graph = Graph(clones)
        n = 0
    ...
    def _is_similar(self, clone1, clone2):
        ...
        a = set(clone1.read_counts)
        b = set(clone2.read_counts)
    ...
    def clones(self) -> List[Tuple[str, List[Cell]]]:
        ...
        def most_abundant_clone_id(cells: List[Cell]):
            counts = Counter()
            for cell in cells:
                counts.update(cell.read_counts)
        ...
        for subgraph in self.graph.connected_components():
            ...
            for cell in cells:
                if highlight is not None and cell.cell_id in highlight:
                    ...
                    highlighting = ''
                clone_ids = sorted(cell.read_counts.keys())
                print(
                    cell.cell_id, highlighting, *clone_ids, sep='\t', file=s)
                counter.update(cell.read_counts.keys())
...
class UncompressedCloneGraph:
    ...
    def _make_cell_graph(self):
        ...
        """
        cells = [cell for cell in self._cells if cell.read_counts]
        graph = Graph(cells)
        for i in range(len(cells)):
            for j in range(i + 1, len(cells)):
                if set(cells[i].read_counts) & set(cells[j].read_counts):
                    # Cell i and j share a clone id
                    graph.add_edge(cells[i], cells[j])
        return graph

    def _make_barcode_graph(self):
        read_counts: Dict[str, int] = Counter()
        for cell in self._cells:
            read_counts.update(cell.read_counts)
        all_clone_ids = list(read_counts)

        # Create a graph of barcodes; add an edge for barcodes occuring in the same cell
        graph = Graph(all_clone_ids)
        for cell in self._cells:
            barcodes = list(cell.read_counts)
