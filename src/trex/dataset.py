...
class DatasetReader:
    ...
    def read_all(self, transcriptome_inputs, amplicon_inputs, names, allowed_cell_ids, require_umis=True, cell_id_tag="CB"):
        ...
        if n_transcriptome == 1:
            reads = self.read_one(transcriptome_inputs[0], self.output_dir / "entries.bam", cell_id_tag, require_umis)
            if n_amplicon == 1:
                reads.extend(
                    self.read_one(amplicon_inputs[0], self.output_dir / "amplicon_entries.bam", cell_id_tag, require_umis)
                )
        else:
            datasets = []
